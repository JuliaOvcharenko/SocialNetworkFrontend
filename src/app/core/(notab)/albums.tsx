						...
						onLongPress={() =>
							Alert.alert("Дії з фото", undefined, [
								...
								},
								{
									...
									style: "destructive",
									onPress: () =>
										onDeletePhoto(photo.albumId, photo.id),
								},
							])
						}
					>
...
export default function AlbumsScreen() {
	...
	const [dotsModal, setDotsModal] = useState<{
		...
		albumId: number | null;
		isShown: boolean;
	}>({ visible: false, albumId: null, isShown: true });

	const { data: albums = [], isLoading: albumsLoading } = useGetAlbumsQuery();

	const systemAlbum = albums.find((a) => a.isDefault === true);
	const customAlbums = albums.filter((a) => a.isDefault === false);
	...
	const handleDeletePhoto = useCallback(
		async (albumId: number, photoId: number) => {
			try {
	...
	const handleTogglePhotoVisibility = useCallback(
		async (photo: Photo) => {
			try {
		...
		<SafeAreaView style={{ flex: 1 }} edges={["bottom", "left", "right"]}>
			<View style={styles.container}>
						...
						<View style={styles.card}>
							<View style={styles.cardHeader}>
								...
								<TouchableOpacity
									...
									onPress={() =>
										handleAddPhoto(systemAlbum.id)
									}
								>
									<GalleryButton
										style={styles.addPhotoBtnIcon}
									/>
				...
				<AlbumDotsModal
					...
					isPublic={dotsModal.isShown}
					onClose={() =>
						setDotsModal((s) => ({ ...s, visible: false }))
					}
...
const styles = StyleSheet.create({
	...
	photoRow: { paddingTop: 12, paddingRight: 8, alignItems: "flex-start" },
	photoCard: {
